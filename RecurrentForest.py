...
class RecurrentForest:
    ...
    def __init__(self, X, y, T, n_trees, p_connect, p_feature, p_example):
        ...
        self.Adjacency = np.random.uniform(0, 1, (n_trees, n_trees)) < p_connect
        # subsets of features
    ...
    def initializeTree(self, time, idx):
        ...
        tree = RandomTree(self.X, self.y, self.tails, *sparse_vectors, t=time, idx=idx)
    ...
    def updateNewTails(self, time, newX):
        for tree in range(self.n_trees):
            self.newTails[:,tree] = self.trees[time][tree].predictNew(newX, time)
...
class RandomTree:
    ...
    """
    Initialize K trees
    Initialize K random vectors theta_i
    Connect each tree to all other trees with probability p

    For tree in forest
        append 0s to X, one for each connected tree

    For i = 1:T
        for tree in forest
            tree.fit(X,y,theta)
        for tree in forest
            X[tail] =  outputs of adjacent
    """

    def __init__(self, X, y, tails, sparse_n, sparse_d, sparse_adj, t=None, idx=None):
        """
        ...
        self.t = t # which round in {1..T} am I on
